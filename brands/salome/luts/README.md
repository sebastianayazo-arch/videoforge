...
The brand LUT (`salome-warm-natural.cube`) is **generated**, not hand-authored.
...
4. **Output.** A 33³ `.cube` LUT written here as `salome-warm-natural.cube`.
...
one exception (`!templates/**/*.cube` covers template LUTs — the Salomé brand
LUT is stored in the secured asset drive and regenerated on demand from the seed
above, so the repo stays binary-free).

> Placeholder: `salome-warm-natural.cube` is not committed. Run the LUT
